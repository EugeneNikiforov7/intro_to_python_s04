...
# обладают разной урожайностью, поэтому ко времени сбора на них выросло различное число ягод — на i-ом кусте выросло ai ягод.
...
# n = 7
# gryadka = [1, 2, 3, 4, 5, 3, 9]

n = int(input('Введите число кустов на грядке: '))
gryadka = (input().split())
summa = []
gryadka.append(gryadka[0])
gryadka.append(gryadka[1])
for i in range(len(gryadka) - 2):
    summa.append(int(gryadka[i]) + int(gryadka[i + 1]) + int(gryadka[i + 2]))
gryadka.pop(-1)
gryadka.pop(-1)

print(n, ' -> ', *gryadka)
print(summa, max(summa))